...
public sealed class LightRequest
{
        ...
        bool castsShadows, long version)
    {
        ...
        ValidateColor(red, nameof(red)); ValidateColor(green, nameof(green));
        ValidateColor(blue, nameof(blue));
        if (version < 0) throw new ArgumentOutOfRangeException(nameof(version));
        RequestId = requestId.Trim(); Kind = kind; Priority = priority;
        WorldX = worldX; WorldY = worldY; WorldZ = worldZ;
        Range = range; Intensity = intensity;
        Red = red; Green = green; Blue = blue;
        CastsShadows = castsShadows; Version = version;
    }
