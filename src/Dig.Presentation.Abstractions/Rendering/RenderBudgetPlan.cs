...
public sealed class RenderBudgetPlan
{
    ...
    {
        Effects = effects; Lights = lights; ParticleCount = particleCount;
        DroppedEffects = droppedEffects; DroppedLights = droppedLights;
    }
        ...
        double focusX, double focusY, double focusZ)
    {
        ...
        ValidateUniqueEffects(effects); ValidateUniqueLights(lights);
        ...
        for (int index = 0; index < orderedEffects.Count; index++)
        {
            ...
            if (selectedEffects.Count >= budget.MaximumEffects
                || particles + effect.ParticleBudget > budget.MaximumParticles) continue;
            selectedEffects.Add(effect); particles += effect.ParticleBudget;
        }
        ...
        return new RenderBudgetPlan(
            ...
            new ReadOnlyCollection<SelectedLight>(selectedLights), particles,
            effects.Count - selectedEffects.Count, lights.Count - selectedLights.Count);
    }
