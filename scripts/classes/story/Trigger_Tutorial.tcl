...
//					tasklist_add this "elf_action -screen \{200 200 16\}"
				...
				"gnome1deselected..." {
					...
					call_method $gnome_ref2 Editor_Set_Info {{gender female} {name Louise}}
...
//					set condition [expr ($condition<0.3&&$hod%3<1)?0:(($condition<0.6)?1:2)]
					...
					set gnomename5 [lindex {Charly Louise Louise Charly} $which]
				...
				"zeltunboxed." {
					...
					call_method $gnome_ref4 Editor_Set_Info {{gender female} {name Fiona}}
...
//		set stones [obj_query this "-class Stein -range 200 -limit 5"]
		...
		proc check_callback {player_task_progress} {
			...
			switch [string trim $player_task_progress "."] {
				...
				"fireplaceonmouse" { if { [vector_equal [get_place_info] "1 0 0"]} {return true} {return false} }
				"fireplacegreen" { if { [vector_equal [get_place_info] "1 1 0"]} {return true} {return false} }
				"fireplacebox" { if { [vector_equal [get_place_info] "1 1 1"]} {return true} {return false} }
				"fireplacewrong" {
				...
				"notmarked" {
					global digwatch1
					if { [vector_equal "-1.0 -1.0 1.0" [dig_next $digwatch1 $gnome_ref1]] } {
						return true
					}
					return false
				}
				...
				"tunneldigged" {
					global digwatch2
					if {[get_hmap [lindex $comp_val 0] [lindex $comp_val 1]]<12&& [vector_equal [dig_next $digwatch2 $gnome_ref1] "-1.0 -1.0 1.0"] } {return true} {return false} }
