...
public sealed class RenderBudgetPlanTests
{
    ...
    public void Critical_effects_win_before_nearer_ambient_effects()
    {
        ...
        {
            Effect("ambient", VfxPriority.Ambient, 0d, particles: 8),
            Effect("critical", VfxPriority.Critical, 100d, particles: 8),
        };
    ...
    public void Particle_budget_is_never_exceeded()
    {
        EffectSpawnRequest[] effects =
        {
            Effect("a", VfxPriority.Important, 1d, particles: 12),
            Effect("b", VfxPriority.Important, 2d, particles: 12),
            Effect("c", VfxPriority.Important, 3d, particles: 4),
        };
        RenderBudgetPlan plan = RenderBudgetPlan.Create(effects,
        ...
        Assert.Equal(new[] { "a", "c" },
            new[] { plan.Effects[0].RequestId, plan.Effects[1].RequestId });
    }
    ...
    public void Stable_id_breaks_equal_priority_and_distance_ties()
    {
        EffectSpawnRequest[] effects =
        {
            Effect("z", VfxPriority.Normal, 1d, particles: 1),
            Effect("a", VfxPriority.Normal, -1d, particles: 1),
        };
        RenderBudgetPlan plan = RenderBudgetPlan.Create(effects,
