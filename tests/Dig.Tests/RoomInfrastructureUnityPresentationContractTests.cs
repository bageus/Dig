...
{
    [Fact]
    public void Runtime_wires_explicit_upgrade_mode_room_overlays_hud_and_input_shielding()
    {
        ...
        string hudMode = Read(runtime, "DigGameHudCanvas.RoomPlanningMode.cs");
        ...
        Assert.Contains("_roomUpgradeMode", roomInput);
        Assert.Contains("SetRoomUpgradeMode(bool enabled)", roomInput);
        Assert.Contains("IsRoomUpgradeModeUnlocked", roomInput);
        Assert.Contains("enabled && !IsRoomUpgradeModeUnlocked", roomInput);
        Assert.Contains("OpenExcavationMenuInDigMode", roomInput);
        ...
        Assert.Contains("PrimitiveType.Cube", renderer);
        ...
        Assert.Contains("Room Types Toggle", hudMode);
        Assert.Contains("SetRoomUpgradeMode(!_interaction.RoomUpgradeMode)", hudMode);
        Assert.DoesNotContain("Dig Mode", hudMode);
        Assert.DoesNotContain("Upgrade Mode", hudMode);
        Assert.Contains("button.interactable = room.CanChangePurpose", hudRoom);
