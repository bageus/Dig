...
internal sealed partial class DigTerrainWorkSession
{
        ...
        out Result result)
    {
        ...
        RequireSpatialExcavationInitialized();
        JobSnapshot? targetJob = _jobRepository.Get().GetAll()
        ...
        EntityId residentId = residents[0];
        Result released = ReleaseSelectedResidentAssignment(residentId, targetJob.Id, tick);
        ...
        JobSystem jobs = _jobRepository.Get();
        targetJob = jobs.Get(targetJob.Id);
        if (targetJob!.Status == JobStatus.Available)
        {
            _candidateProvider!.SetCandidates(targetJob.Id, NoCandidates);
            result = jobs.Claim(targetJob.Id, residentId, tick);
            if (result.IsFailure)
            {
                return true;
            }
        ...
        else if (targetJob.AssignedAgentId != residentId)
        {
    ...
    private Result ReleaseSelectedResidentAssignment(
        EntityId residentId,
        EntityId targetJobId,
        long tick)
    {
        JobSnapshot? current = _jobRepository.Get().GetAll()
            .FirstOrDefault(value => value.Id != targetJobId
                && !value.IsTerminal
