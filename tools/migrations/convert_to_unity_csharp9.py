...
def iter_csharp_files() -> list[Path]:
    ...
    return sorted(path for path in files if "obj" not in path.parts and "bin" not in path.parts)
...
def update_build_properties() -> bool:
    ...
    updated = updated.replace("<ImplicitUsings>enable</ImplicitUsings>", "<ImplicitUsings>disable</ImplicitUsings>")
    if updated == text:
        return False
