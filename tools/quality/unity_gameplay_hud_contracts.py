...
def check_gameplay_hud_and_work_contracts(
    ...
    movement_index = loop_text.find("PlanMovement(before, nextTick)")
    ...
    if movement_index < 0 or assignment_index >= movement_index:
        errors.append(
            f"{loop_path}: assignment must run before movement"
        )
    errors.extend(reject_fragments(loop_path, loop_text, "removed demo stockpile hauling loop", ("SynchronizeHauling(", "AdvanceHauling(")))
    errors.extend(reject_fragments(
        loop_path,
        loop_text,
