...
- save format v11 with `v10 -> v11` migration and deterministic continuation fields;
...
The system remains `IMPLEMENTATION_IN_PROGRESS` until PR #529 is reconciled with current `main` and the final Quality workflow completes. `VERIFIED` additionally requires licensed Unity Test Runner evidence; source-contract or skipped activation-gate results are not sufficient.
