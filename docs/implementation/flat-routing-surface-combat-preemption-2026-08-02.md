...
- the first combat-preemption correction could recreate self-defense after a direct command cancelled combat, contradicting the confirmed rule that direct player commands are always highest priority.
...
4. The initial preemption gate treated every persistent incoming enemy intent as a reason to recreate resident self-defense. After `PrepareResidentsForDirectCommand` cancelled resident combat, the next autonomy evaluation could therefore take control back from the player's order.
5. Existing direct commands had authoritative manual-movement and assigned-job owners, but the combat/needs gate did not consume those owners as an input-priority fact.
