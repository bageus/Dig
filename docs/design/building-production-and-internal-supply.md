...
- production and supply of one building share one building-level operation reservation and cannot be claimed/active concurrently; after production return, reachable enabled refill is scheduled before the next order;
...
- production и internal-stock refill одного building никогда не активны одновременно; после close+return scheduler создаёт remote refill route до запуска следующего order;
