...
При наличии non-terminal production queue operation owner чередуется по authoritative per-building turn: runnable Production unit -> не более одного successful Supply batch -> следующая runnable Production unit. Supply не заполняет весь internal stock перед возвратом к production. Если следующий order ещё не имеет полного required input set, Supply выполняется batch-by-batch до runnable состояния; после этого Production немедленно получает следующий turn. Без production queue enabled refill продолжает planning до `current + incoming == capacity`. Недоступный source или failed batch не блокирует runnable production: source quantity, resident slot claims и incoming capacity освобождаются атомарно, turn пересчитывается, а следующий synchronization pass может продолжить производство либо выбрать другой supply source. Полный cross-system contract: [`runtime-needs-supply-sleep-food-recovery.md`](runtime-needs-supply-sleep-food-recovery.md).
...
- production and supply of one building share one building-level operation reservation and cannot be active concurrently; with a runnable queue they alternate one Production unit and at most one Supply batch, while an unrunnable order permits batch-by-batch refill only until it becomes runnable;
...
- production и internal-stock refill одного building никогда не активны одновременно; при runnable queue после close+return scheduler допускает не более одного refill batch и затем запускает следующий production unit;
