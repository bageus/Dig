...
Индивидуальных вкусовых профилей, taste vectors и наследуемых food preferences **нет**. Все исследованные блюда имеют равный базовый вес выбора. #144 закрыта как ненужное расширение.
...
Положительный базовый Mood подходящей еды **может поднять Mood выше 50**. Специального food cap `50` нет: результат ограничивается только текущим `MoodMaximum` конкретного resident. То же правило применяется к положительному бонусу разнообразия.
...
- положительный diversity delta может поднять Mood выше 50 и ограничивается только текущим `MoodMaximum`;
...
- positive food/diversity Mood не имеет отдельного cap 50;
...
Inspector показывает desired/consumed/match, visibility/fallback reason, source reservation, bite progress, Nutrition/Mood effects, history 10, Matches/Mismatches, researched dish count, formula branch, текущий MoodMaximum, kitchen tier, worker, speed multiplier и block reason.
